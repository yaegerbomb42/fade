...
const MessageInputPanel = ({ onSendMessage, activeChannel, isTyping, onTypingChange }) => {
  ...
  const [tempBanEnd, setTempBanEnd] = useState(0);
  ...
  useEffect(() => {
    ...
    // Load violation count and temp ban status
    const savedViolations = localStorage.getItem('fade-violations');
    const savedTempBan = localStorage.getItem('fade-temp-ban-end');
    ...
    if (savedTempBan) {
      const banEndTime = parseInt(savedTempBan);
      if (banEndTime > Date.now()) {
        setTempBanEnd(banEndTime);
        const remainingTime = Math.ceil((banEndTime - Date.now()) / 1000);
  ...
  const handleMessageChange = (e) => {
    ...
    // Limit each line to 40 characters for display purposes
    const validLines = lines.map(line => line.length <= maxCharsPerLine ? line : line.substring(0, maxCharsPerLine));
    const limitedMessage = validLines.join('\n');
  ...
  const applyTempBan = (violations) => {
    ...
    const banIndex = Math.min(violations - 1, banDurations.length - 1);
            ...
            {message.length > 0 && (
              <div className="absolute bottom-2 right-2 text-xs text-text-secondary font-mono">
                {(() => {
                  ...
                  return `${currentLine.length}/40 • ${lines.length}/4`;
                })()}
              </div>
            )}
