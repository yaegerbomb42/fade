...
const ChannelSelector = ({ onChannelChange, activeChannel, channelUserCounts = {} }) => {
  ...
  const defaultChannels = [
    ...
    { id: 'study', name: 'Study Together' },
    { id: 'pets', name: 'Pets & Animals' },
    { id: 'late-night', name: 'Late Night Chat' }
  ];
    ...
    <div className="fixed top-4 left-4 z-interface">
      {collapsed ? (
        ...
        <div className="glass-panel p-4 w-52 space-y-2 fade-in">
          <div className="mb-3">
            <h3 className="text-sm font-semibold text-text-primary mb-1">Choose Your Vibe</h3>
            <p className="text-xs text-text-secondary opacity-80">Join others doing the same thing</p>
          </div>
          {defaultChannels.map((channel) => (
            <button
              key={channel.id}
              onClick={() => handleChannelSelect(channel)}
              className={`glass-button w-full px-3 py-2.5 text-sm flex justify-between items-center transition-all duration-300 ${activeChannel?.id === channel.id ? 'bg-primary/40 text-text-primary border-primary/50' : 'text-text-secondary hover:text-text-primary hover:bg-glass-highlight'}`}
            >
              <span className="font-medium">{channel.name}</span>
              <span className="text-xs opacity-70 font-mono bg-glass-surface/50 px-2 py-0.5 rounded-full">{channel.users}</span>
            </button>
          ))}
