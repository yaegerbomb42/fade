...
const MainChatInterface = () => {
  ...
  // Simplified highway lane system to prevent performance issues
  const findAvailablePosition = useCallback((messageId, messageText = '', preferredLane = null) => {
    const lanes = 8; // Reduced back to 8 lanes for stability
    const usableHeight = 70; // Use 70% of screen height
    const topMargin = 15; // Start lanes at 15% from top
    const laneHeight = usableHeight / lanes; // ~8.75% per lane
    
    // Simplified message dimensions
    const messageLength = messageText.length || 50;
    const messageWidth = Math.min(Math.max(messageLength * 0.5, 20), 35); // 20-35% width
    const messageHeight = 5; // Fixed height for simplicity
    
    // Get current active positions (simplified)
    const activePositions = Array.from(messagePositions.current.values())
      .filter(pos => pos.left > -10 && pos.left < 110); // Only visible messages
    
    // Try lanes in order
    const lanesToTry = Array.from({length: lanes}, (_, i) => i);
    if (preferredLane !== null && preferredLane < lanes) {
      lanesToTry.unshift(preferredLane);
    }
    
    for (const laneIndex of lanesToTry) {
      const laneTop = topMargin + (laneIndex * laneHeight);
      
      // Check if this lane position is clear
      const isLaneClear = !activePositions.some(pos => {
        const verticalDistance = Math.abs(pos.top - laneTop);
        const horizontalDistance = Math.abs(pos.left - 105);
        return verticalDistance < 10 && horizontalDistance < 30; // Simple collision check
      });
      ...
      if (isLaneClear) {
        const position = {
          lane: laneIndex,
          top: laneTop,
          left: 105 + Math.random() * 3, // Small randomization
          horizontalStart: 105,
          animationSpeed: 2 + Math.random() * 0.5,
          messageWidth,
          messageHeight,
          createdAt: Date.now(),
          reservedSpace: {
            topBound: laneTop - 5,
            bottomBound: laneTop + 5,
            leftBound: 105,
            rightBound: -15,
            width: messageWidth,
  ...
  // Highway traffic control - real-time position updates with lane discipline
  const updateMessagePosition = useCallback((messageId, newLeft) => {
    const position = messagePositions.current.get(messageId);
    if (!position) return;
    
    // Update position maintaining lane discipline
    const updatedPosition = { ...position, left: newLeft };
    
    // Check if this movement would cause a highway collision
    const otherTraffic = Array.from(messagePositions.current.entries())
      .filter(([id, _]) => id !== messageId)
      .map(([_, pos]) => pos);
    
    const wouldCauseAccident = otherTraffic.some(otherCar => {
      // Calculate actual collision bounds
      const thisCarBounds = {
        top: updatedPosition.top - (updatedPosition.messageHeight || 5) / 2,
        bottom: updatedPosition.top + (updatedPosition.messageHeight || 5) / 2,
        left: newLeft - (updatedPosition.messageWidth || 25) / 2,
        right: newLeft + (updatedPosition.messageWidth || 25) / 2
      };
      
      const otherCarBounds = {
        top: otherCar.top - (otherCar.messageHeight || 5) / 2,
        bottom: otherCar.top + (otherCar.messageHeight || 5) / 2,
        left: otherCar.left - (otherCar.messageWidth || 25) / 2,
        right: otherCar.left + (otherCar.messageWidth || 25) / 2
      };
      
      // Check for overlap (collision)
      const verticalOverlap = !(thisCarBounds.bottom < otherCarBounds.top || 
                               thisCarBounds.top > otherCarBounds.bottom);
      const horizontalOverlap = !(thisCarBounds.right < otherCarBounds.left || 
                                 thisCarBounds.left > otherCarBounds.right);
      
      return verticalOverlap && horizontalOverlap;
    });
    
    if (!wouldCauseAccident) {
      // Safe to update position
      messagePositions.current.set(messageId, updatedPosition);
    } else {
      // Emergency lane change - slight vertical adjustment to avoid collision
      const laneChangeAmount = (Math.random() - 0.5) * 8; // ±4% emergency adjustment
      const emergencyPosition = {
        ...updatedPosition,
        top: Math.max(10, Math.min(85, updatedPosition.top + laneChangeAmount))
      };
      messagePositions.current.set(messageId, emergencyPosition);
      console.log(`Emergency lane change for message ${messageId.substring(0, 8)}`);
    }
  }, []);
