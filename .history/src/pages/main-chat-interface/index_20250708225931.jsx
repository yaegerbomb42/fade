...
const MainChatInterface = () => {
  ...
  useEffect(() => {
    ...
    currentIds.forEach(id => {
      // Only remove IDs that are clearly very old timestamps AND numeric
    ...
    const changeListener = onChildChanged(messagesRef, (snapshot) => {
      ...
      setMessages(prev => {
        ...
        if (messageExists) {
          return prev.map(m => (m.id === id ? { ...m, reactions: updated.reactions || m.reactions } : m));
        } else {
          return prev;
        }
      });
  ...
  const handleReaction = useCallback((messageId, reactionType) => {
    ...
    const reactionRef = ref(database, `channels/${activeChannel.id.replace(/[.#$[\]]/g, '_')}/messages/${messageId}/reactions/${reactionType}`);
