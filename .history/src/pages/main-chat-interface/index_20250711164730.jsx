...
import { setupAdDebugging, debugAdIssues } from '../../utils/adDebugger';
...
const AdsterraBanner = () => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      setTimeout(checkAndStyleAd, 2000); // Retry after 2 seconds
    };
    script.onerror = (error) => {
      console.warn('AdsterraBanner: Script failed to load', error);
      // Try alternative loading method after error
      setTimeout(() => {
        if (!document.getElementById('container-58d94318819023c51d2375249b2d6604')) {
          console.log('AdsterraBanner: Retrying with different approach...');
          // Could add fallback ad network here
        }
      }, 3000);
    };
...
const SocialBar = () => {
  ...
  return (
    <div className={`social-bar-container ${isMobileDevice ? 'mobile-social' : 'desktop-social'}`} id="social-bar-container">
      {!isLoaded && !hasError && (
        <div 
          className="glass-panel p-2 text-xs text-text-secondary bg-glass-surface/20" 
          style={{ 
            minWidth: isMobileDevice ? '100px' : '120px',
            fontSize: isMobileDevice ? '9px' : '12px',
            padding: isMobileDevice ? '4px' : '8px'
          }}
        >
          <div className="flex items-center gap-1">
            <div className={`animate-spin rounded-full border-b-2 border-accent ${isMobileDevice ? 'h-2 w-2' : 'h-3 w-3'}`}></div>
            <span>{isMobileDevice ? 'Loading...' : 'Loading social ads...'}</span>
          </div>
        </div>
      )}
      {hasError && (
        <div 
          className="glass-panel p-2 text-xs text-error bg-error/20"
          style={{ 
            fontSize: isMobileDevice ? '9px' : '12px',
            padding: isMobileDevice ? '4px' : '8px'
          }}
        >
          <div className="flex items-center gap-1">
            <Icon name="AlertCircle" size={isMobileDevice ? 10 : 12} />
            <span>{isMobileDevice ? 'Social failed' : 'Social ads failed'}</span>
          </div>
        </div>
      )}
      {isLoaded && (
        <div 
          className="glass-panel p-1 text-xs text-success bg-success/20"
          style={{ 
            fontSize: isMobileDevice ? '9px' : '12px',
            padding: isMobileDevice ? '4px' : '8px'
          }}
        >
          <div className="flex items-center gap-1">
            <Icon name="Check" size={isMobileDevice ? 10 : 12} />
            <span>{isMobileDevice ? 'Social loaded' : 'Social ads loaded'}</span>
          </div>
        </div>
      )}
    </div>
...
const MainChatInterface = () => {
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      {/* Debug button for ad troubleshooting (only in development) */}
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed top-4 right-4 z-interface">
          <button
            onClick={() => debugAdIssues()}
            className="glass-button p-2 text-xs text-text-secondary hover:text-text-primary transition-colors"
            title="Debug Ads"
          >
