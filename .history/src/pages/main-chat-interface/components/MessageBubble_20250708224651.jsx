...
const MessageBubble = ({ message, index, onReaction, onRemove, activityLevel = 1 }) => {
  ...
  const [position, setPosition] = useState({
    // Distribute messages vertically in 4 lanes to reduce overlap
    top: (index % 4) * 20 + 10 + Math.random() * 15, // 10%, 30%, 50%, 70% plus some randomness
    left: 100 + Math.random() * 20, // Start off-screen to the right (100% to 120%)
  });
  const [isVisible, setIsVisible] = useState(false);
  const [animationDuration, setAnimationDuration] = useState('25s'); // Slower default duration
