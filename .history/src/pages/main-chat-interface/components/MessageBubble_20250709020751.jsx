...
const MessageBubble = ({ message, index, onReaction, onRemove, activityLevel = 1, totalMessages = 0 }) => {
  ...
  const [position, setPosition] = useState(() => {
    // Use server-provided position if available, otherwise fallback to client-side
    if (message.position) {
      const { lane, verticalOffset, horizontalStart } = message.position;
      const laneHeight = 70 / 6; // 6 lanes
    ...
    // Fallback for messages without server positioning
    const lanes = 6;
