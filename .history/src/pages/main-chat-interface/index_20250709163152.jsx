...
const MainChatInterface = () => {
  ...
  useEffect(() => {
    ...
    const { timeUntilNext } = getGlobalForeverStreamTimestamp();
    ...
    const startTimeout = setTimeout(() => {
      const syncedInterval = setInterval(() => {
        setMessages(current => {
          ...
          if (current.length < MAX_ACTIVE_MESSAGES && currentChannelMessages.length > 0) {
            const { cycleNumber: currentCycle } = getGlobalForeverStreamTimestamp();
            const messageIndex = currentCycle % currentChannelMessages.length;
            const messageToAdd = currentChannelMessages[messageIndex];
            
            if (!messageToAdd) return current;
            
            // Create unique cycle ID to prevent duplicates
            const cycleId = `${messageToAdd.id}_cycle_${currentCycle}_${activeChannel.id}`;
            ...
            if (!messageToAdd.text || messageToAdd.text.trim() === '' ||
              ...
              console.warn('Skipping invalid forever stream message:', messageToAdd);
              return current;
            }
