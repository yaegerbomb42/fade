...
const MainChatInterface = () => {
  ...
  const FOREVER_STREAM_CYCLE_INTERVAL = 35000; // 35 seconds between messages (much much slower)
  const MAX_FOREVER_STREAM_MESSAGES = 5000;
  const MAX_ACTIVE_MESSAGES = 4; // Much fewer messages on screen for smoothest experience
  ...
  const calculateMessagePosition = (messageId, cycleNumber, cycleProgress) => {
    ...
    // Calculate horizontal position based on cycle progress with smoother movement
    // Messages start at 110% and move to -20% over the cycle duration
    const startX = 110;
    const endX = -20;
    
    // Use easeInOut function for smoother movement
  ...
  useEffect(() => {
    ...
    const updateForeverStream = () => {
      setMessages(current => {
        ...
        for (let i = 0; i < MAX_ACTIVE_MESSAGES && i < currentChannelMessages.length; i++) {
          const messageOffset = i * 4; // Much larger spacing between messages
          ...
          // Calculate the progress for this specific message with larger offset
          const messageProgress = (cycleProgress + (messageOffset * 0.2)) % 1;
          
          // Only show messages that are still visible (haven't completely exited)
          if (messageProgress > 0.85) continue; // Message has exited even earlier
    ...
    // Update much less frequently to reduce lag - every 2 seconds
    const updateInterval = setInterval(updateForeverStream, 2000);
