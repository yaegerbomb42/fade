...
const MainChatInterface = () => {
  ...
  const FOREVER_STREAM_CYCLE_INTERVAL = 12000; // 12 seconds between messages (much slower)
  const MAX_FOREVER_STREAM_MESSAGES = 5000;
  const MAX_ACTIVE_MESSAGES = 15; // Fewer messages on screen at once
  ...
  useEffect(() => {
    ...
    const loadRecentMessages = async () => {
      try {
        // For regular channels, load messages from the last 10 minutes
        ...
        if (snapshot.exists()) {
          ...
          // Add these existing messages to the queue for display
          messageArray.forEach(message => {
            if (!permanentlyProcessedIds.current.has(message.id)) {
              permanentlyProcessedIds.current.add(message.id);
              setMessageQueue(q => [...q, { ...message, channelId, isRecentMessage: true }]);
            }
          });
  ...
  useEffect(() => {
    ...
    const updateForeverStream = () => {
      setMessages(current => {
        ...
        for (let i = 0; i < MAX_ACTIVE_MESSAGES && i < currentChannelMessages.length; i++) {
          ...
          // Calculate the progress for this specific message
          const messageProgress = (cycleProgress + messageOffset) % 1;
          
          // Only show messages that are still visible (haven't completely exited)
          if (messageProgress > 0.95) continue; // Message has exited
    ...
    // Update every 100ms for smooth positioning
    const updateInterval = setInterval(updateForeverStream, 100);
