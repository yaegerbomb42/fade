...
import Icon  // Clean up old messages based on flow position, not time
  // Keep the flow continuous by maintaining last 20 messages in sequence
  useEffect(() => {
    const interval = setInterval(() => {
      // Remove older messages to prevent memory buildup
      // Keep the most recent 20 messages in the continuous flow
      setMessages(prev => {
        if (prev.length <= 20) return prev;
        
        // Sort by timestamp and keep most recent 20 for smooth flow
...
const MainChatInterface = () => {
  ...
  // POSITION-BASED MESSAGING ARCHITECTURE:
  // - Messages flow continuously based on their sequence position
  // - Refreshing shows the current flow state, not just recent messages
  // - Messages persist in their flow position regardless of user refresh
  // - Only messages with likes are saved to history for "Top Vibes"
  // - Each channel maintains its continuous flow independently
  useEffect(() => {
    ...
    // Load the current flow state - last 25 messages to show continuous flow
    // This ensures refresh doesn't break the flowing progression
    const currentFlowQuery = query(
      messagesRef,
      orderByChild('timestamp'),
      limitToLast(25) // Get last 25 messages to maintain flow continuity
    );

    const addListener = onChildAdded(currentFlowQuery, (snapshot) => {
      ...
      // Add all messages from the current flow to maintain continuity
      // No time-based filtering - position-based flow persistence
    ...
    const changeListener = onChildChanged(currentFlowQuery, (snapshot) => {
    ...
    return () => {
      off(currentFlowQuery, 'child_added', addListener);
      off(currentFlowQuery, 'child_changed', changeListener);
    };
