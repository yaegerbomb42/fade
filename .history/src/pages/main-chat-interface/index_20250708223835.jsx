...
const MainChatInterface = () => {
    ...
    const processQueue = () => {
      setMessageQueue(prev => {
        ...
        setMessages(msgs => {
          ...
          if (!exists && !permanentlyProcessedIds.current.has(next.id)) {
            const minDuration = 4;
            ...
            // Mark message as processed permanently
            permanentlyProcessedIds.current.add(next.id);
          }
          return msgs;
        });
  ...
  useEffect(() => {
    ...
    // Clean up very old permanently processed IDs (older than 10 minutes) to free memory
    ...
    currentIds.forEach(id => {
      // Remove very old numeric IDs (likely timestamps)
      if (!isNaN(id) && parseInt(id) < tenMinutesAgo) {
    ...
    const addListener = onChildAdded(messagesRef, (snapshot) => {
      ...
      // Debug logging
      console.log('Firebase message received:', { id, text: newMessage?.text, 
        alreadyProcessed: processedIds.current.has(id), 
        permanentlyProcessed: permanentlyProcessedIds.current.has(id) });
      
      if (!processedIds.current.has(id) && !permanentlyProcessedIds.current.has(id)) {
    ...
    const changeListener = onChildChanged(messagesRef, (snapshot) => {
      ...
      // Only update if message is still in current stream and not permanently processed
  ...
  const handleRemoveMessage = useCallback((id) => {
    console.log('Removing message permanently:', id);
